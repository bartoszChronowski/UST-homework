    ...
    it.each(examples)(
        ...
        async ({ password, passedRules }) => {
          ...
          passedRules.forEach((rule) => {
            expect(
                wrapper.get(`[data-test-rule-indicator="${rule}"]`).classes(RULE_INDICATOR_CLASS.Pass)
            ).toBe(true)
            expect(
                wrapper.get(`[data-test-rule-indicator="${rule}"]`).classes(RULE_INDICATOR_CLASS.Fail)
            ).toBe(false)
          })
